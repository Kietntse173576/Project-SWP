...
export default function Cart() {
  ...
  const handleProceedToCheckout = () => {
    ...
    if (!accessToken) {
        notification("warning", 'Bạn phải đăng nhập trước khi tiến hành thanh toán.');
    } else {
      ...
      <div className="cart p-4 bg-white shadow-lg rounded-lg w-3/4">
        ...
        {cartItems.length > 0 ? (
          <div className="flex justify-between">
            <div className="w-2/3">
              {cartItems.map((item) => (
                <div className="cart-item flex mb-4" key={`${item.id}-${item.code}-${item.price}-${item.quantity}`}>
                  <img src={item.image} alt={item.name} className="w-36 h-36" />
                  <div className="item-details ml-4">
                    <Typography variant="h6">{item.name}</Typography>
                    <Typography variant="body2">MSP: {item.productName}</Typography>
                    ...
                    <div className="flex mt-2">
                      <Button onClick={() => handleRemoveFromCart(item)}>Xóa</Button>
                    </div>
                  </div>
                </div>
              ))}
            ...
            <div className="summary w-1/3">
              <Typography variant="h6" className="mb-2">Tổng tiền</Typography>
              <Divider />
              <div className="flex justify-between">
              ...
              <Button type="primary" className="w-full mt-2 bg-blue-500 h-[50px]" onClick={handleProceedToCheckout}>TIẾN HÀNH ĐẶT HÀNG</Button>
            </div>
          </div>
        ) : (
